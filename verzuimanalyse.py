...
def analyse_verzuim(filename, contents, sbi_code='6420', periode='2024KW2'):
    ...
    markdown_lines = [
        "### Verzuimanalyse",
        "- Intern verzuim: **" + str(verzuimpercentage) + "%**",
        "- CBS Benchmark (SBI " + sbi_code + ", " + periode + "): **" + str(benchmark) + "%**",
        "- Afwijking: **" + ("+" if afwijking > 0 else "") + str(round(afwijking, 1)) + "%**",
        "- Risiconiveau: **" + risico + "**",
        "",
        "**Interpretatie:**",
        "Het verzuim ligt " + ("+" if afwijking > 0 else "") + str(round(afwijking, 1)) + "% " + ("boven" if afwijking > 0 else "onder") + " het branchegemiddelde. Risico wordt als **" + risico.upper() + "** beoordeeld.",
        "",
        "**Aanbevelingen:**"
    ] + ["- " + a for a in aanbevelingen] + ["", "**Vervolgstappen:**"] + ["- " + v for v in vervolgstappen]
    ...
    return {
        ...
        "afwijking": afwijking,
        "interpretatie_markdown": markdown
    }
